        ...
        success: function(response){
            ...
            if ( json == true){
                $('main').html('<section  class="QR">\
                    <div id="connectBox">\
                        <h3>Kontakt uppdaterad</h3>\
                        <div id="connectionDone"></div>\
                    </div>\
                </section>\
                <section id="contacts">\
                     <figure>\
        ...
        success: function(response){
            ...
            if (json !== false){
                ...
                $('#handleEveryLabel').on('click',function(){
                    if( $('#handleEveryLabel').html() == 'Ändra taggar'){
                        $('#handleEveryLabel').text(function(){
                            return 'Klar'
                        });
                    }
                    else{
                    ...
                    $('#tags, #edithLabels').slideToggle(300);
                });

                $('#addInfo').click(function(){
                    $('#handleEveryLabel').slideToggle(300);
                    $('#infoBox').slideToggle(300);
                });

                var labelList = []
                $.each(json.connections.labels, function(i, tag){
                    labelList.push(tag.id);
                });
                ...
                $.each(labels, function(index, label){
                    ...
                    if ( $.inArray(label.id, labelList) > -1 ){
                        $('#tags').prepend('<li class="tags chosenTag" data-id="'+ label.id + '">\
                            <p>'+label.text +'</p>\
                        </li>');
                    }
                    else{
                        $('#tags').prepend('<li class="tags" data-id="'+ label.id + '">\
                            <p>'+label.text +'</p>\
                        </li>');
                    }
                    return labels;
                ...
                $('#saveInfoBtn').click(function(){
                    ...
                    var connectionId = json.connections.id;
...
var addLable = (function(){
    ...
    var createLabelNode = function(label){
        ...
        $('#edithLabels').find('ul').prepend(newLabelEdith);
        $('#tags').prepend(newLabel);
        deleteLabel.eventHandler();
    };
